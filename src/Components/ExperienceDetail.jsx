      ...
      <div>
        {experienceData.map((exper, index) => {
          ...
          return (
            <div
              key={index}
              className="mb-6 lg:w-[896px] m-auto lg:flex lg:justify-between lg:mt-6 bg-[#fff] p-8 rounded-lg shadow-exp-shad hover:scale-[1.03] ease-in-out duration-300 cursor-pointer dark:bg-[#1F2937]"
            >
              <div>
                <div className="mb-4 lg:w-[206px] ">{exper.icon}</div>
              </div>
              <div className="flex flex-col-reverse lg:w-3/4 lg:flex lg:justify-between lg:flex-row">
                <div className="lg:w-[80%]">
                  <h1 className="mb-4 text-xl text-title-black lg:text-xl font-semibold lg:mb-4 lg:mx-[-25px] dark:text-blacktheme-bigtext">
                    {exper.exp}
                  </h1>
                  <ul className="ml-5">
                    {exper.ability.map((aby, index) => (
                      <li
                        key={index}
                        className="text-text-gray list-disc text-base dark:text-blacktheme-text"
                      >
                        {aby}
                      </li>
                    ))}
